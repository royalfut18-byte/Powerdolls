...
const collageCards = [
  {
    ...
    className: 'top-4 left-0 w-40 rotate-[-7deg] bg-[#fff4e6] md:w-52',
  },
  {
    title: 'Hero doll portrait',
    path: '/images/power-dolls-hero.jpg',
    className: 'top-28 right-0 w-44 rotate-[6deg] bg-[#fff] md:w-60',
  },
  {
    title: 'Pattern detail board',
    path: '/images/power-dolls-patterns.jpg',
    className: 'bottom-3 left-12 w-48 rotate-[-3deg] bg-[#f7f0ff] md:w-56',
  },
];
      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        ...
        >
          ...
          >
            {[
              ...
              <div key={item} className="stitched-card bg-white/60 px-4 py-4 text-sm leading-6 text-[rgba(29,19,33,0.76)]">
                {item}
              </div>
            ))}
          </motion.div>
        ...
        <div className="relative min-h-[38rem] lg:min-h-[46rem]">
          <motion.div
            ...
            className="absolute left-[6%] top-[8%] h-40 w-40 rounded-[40%_60%_60%_40%/46%_47%_53%_54%] bg-[#ff5a97]/25 blur-2xl"
          />
          <motion.div
            animate={reduceMotion ? undefined : { ...drift, transition: { ...drift.transition, delay: 0.4 } }}
            className="absolute right-[3%] top-[20%] h-48 w-48 rounded-full bg-[#74d8ff]/25 blur-2xl"
          />
          <motion.div
            animate={reduceMotion ? undefined : { y: [0, -10, 0], transition: { duration: 7, repeat: Number.POSITIVE_INFINITY, ease: [0.42, 0, 0.58, 1] as const } }}
            className="hero-panel absolute inset-x-0 top-10 rounded-[2rem] px-5 py-6 md:px-8 md:py-8"
          >
              ...
              <div className="relative min-h-[20rem]">
                {collageCards.map((card, index) => (
                  <motion.figure
                    ...
                    className={`absolute rounded-[1.4rem] border border-[rgba(17,18,22,0.08)] p-3 shadow-[0_22px_55px_rgba(30,19,31,0.18)] ${card.className}`}
                  >
                    <div className="relative flex h-56 flex-col justify-between overflow-hidden rounded-[1rem] border border-[rgba(17,18,22,0.08)] bg-[linear-gradient(135deg,_rgba(255,255,255,0.9),_rgba(255,255,255,0.3))] p-4">
                      ...
                      <div className="relative">
                        <p className="font-display text-2xl leading-tight text-[var(--ink)]">{card.title}</p>
                        <p className="mt-3 font-mono text-xs text-[rgba(29,19,33,0.62)]">{card.path}</p>
                      </div>
                    </div>
                  </motion.figure>
                ))}
          ...
          <motion.div
            animate={
              reduceMotion
                ? undefined
                : { y: [0, -12, 0], transition: { duration: 6, repeat: Number.POSITIVE_INFINITY, ease: [0.42, 0, 0.58, 1] as const } }
            }
            className="absolute bottom-2 left-0 rounded-[1.8rem] bg-[var(--yellow)] px-5 py-4 shadow-[0_22px_45px_rgba(255,188,48,0.35)]"
          >
          ...
          <motion.div
            animate={
              reduceMotion
                ? undefined
                : { y: [0, 8, 0], transition: { duration: 5.5, repeat: Number.POSITIVE_INFINITY, ease: [0.42, 0, 0.58, 1] as const, delay: 0.4 } }
            }
            className="absolute bottom-20 right-3 rounded-[999px] bg-[var(--hot-pink)] px-4 py-2 text-sm font-semibold text-white shadow-[0_18px_45px_rgba(255,90,151,0.34)]"
          >
